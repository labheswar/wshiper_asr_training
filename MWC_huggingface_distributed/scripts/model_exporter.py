...
class ModelExporter:
    ...
    def _export_onnx(
        ...
        """
        Export model to ONNX format with FP16 and INT8 quantization support
        ...
        # Get model dtype for proper dummy input creation
        model_dtype = next(model.parameters()).dtype
        logger.info(f"Model dtype: {model_dtype}")
        
        # Convert model to float32 for ONNX export compatibility
        # ONNX export works best with float32, and ONNX has its own quantization later
        logger.info("Converting model to float32 for ONNX export...")
        original_dtype = model_dtype
        
        # Create a copy of encoder in float32 to avoid modifying original model
        encoder_for_export = model.model.encoder.float()
        
        # Export encoder
        logger.info("Exporting encoder to ONNX...")
        encoder_path = output_path / "encoder.onnx"
        
        # Create dummy input for encoder (matching float32)
        ...
        try:
            ...
            logger.info(f"  Original dtype: {original_dtype}, Export dtype: float32")
        except Exception as e:
            logger.error(f"Encoder ONNX export failed: {e}")
            raise
        ...
        # FP16 Quantization (NEW)
        if self.export_config.get('onnx_quantize_fp16', False):
            try:
                logger.info("Quantizing ONNX model to FP16...")
                ...
                # Save FP16 model
                encoder_fp16_path = output_path / "encoder_fp16.onnx"
                onnx.save(onnx_model_fp16, str(encoder_fp16_path))
                
                logger.info(f"✓ FP16 quantized model saved to: {encoder_fp16_path}")
                export_results['encoder_fp16'] = str(encoder_fp16_path)
                
                # Get size reduction
                original_size = encoder_path.stat().st_size / (1024 * 1024)
                fp16_size = encoder_fp16_path.stat().st_size / (1024 * 1024)
                reduction = ((original_size - fp16_size) / original_size) * 100
                
                logger.info(f"  Original size: {original_size:.2f} MB")
                logger.info(f"  FP16 size: {fp16_size:.2f} MB")
                logger.info(f"  Size reduction: {reduction:.1f}%")
                
            except ImportError:
                logger.warning("onnxconverter-common not installed - skipping FP16 quantization")
        ...
        # INT8 Quantize ONNX model if requested
        if self.export_config.get('onnx_quantize_int8', False):
            try:
                from onnxruntime.quantization import quantize_dynamic, QuantType
                logger.info("Quantizing ONNX model to INT8...")
                
                quantized_encoder_path = output_path / "encoder_int8.onnx"
                ...
                logger.info(f"✓ INT8 quantized model saved to: {quantized_encoder_path}")
                export_results['encoder_int8'] = str(quantized_encoder_path)
