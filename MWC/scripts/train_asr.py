...
class MWCASRTrainer:
    ...
    def __init__(self, config_path: str):
        ...
        self.checkpoints_dir = self.project_root / self.config['checkpoints']['output_dir']
        self.final_models_dir = self.project_root / self.config['export']['final_model_dir']
        ...
        self.metrics = {}
        if self.config['metrics']['compute_wer']:
            self.metrics['wer'] = evaluate.load("wer")
        if self.config['metrics']['compute_cer']:
            self.metrics['cer'] = evaluate.load("cer")
        if self.config['metrics']['compute_bleu']:
            self.metrics['bleu'] = evaluate.load("sacrebleu")
        if self.config['metrics']['compute_chrf']:
    ...
    def prepare_dataset(self):
        ...
        if self.config['data']['use_huggingface']:
    ...
    def train(self):
        ...
        logger.info(f"  FP16: {self.config['training']['fp16']}")
        logger.info(f"  Save every: {self.config['checkpoints']['save_every_n_epochs']} epochs")
        
        # Data collator with FP16 support
        ...
        train_result = trainer.train(
            resume_from_checkpoint=self.config['checkpoints'].get('resume_from_checkpoint')
        )
